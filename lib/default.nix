...
with lib; {
  ...
  config.flake.lib.mkHome = {system ? "x86_64-linux", ...} @ args:
    withSystem system
    ({pkgs, ...}: (inputs.home-manager.lib.homeManagerConfiguration
      ((filterAttrs (k: v: k != "system")
  ...
  config.flake.lib.mkSystem = {system ? "x86_64-linux", ...} @ args:
    withSystem system
    ({pkgs, ...}: (inputs.nixpkgs.lib.nixosSystem
      (args
        // {
          inherit pkgs;
